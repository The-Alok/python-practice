...
name = input("\nDeviyon aur sajjano, namaskar! Main Amitabh Bachchan swagat karta hoon aap sabhi ka Kaun Banega Crorepati mein!\nGame start karne se pahle kripaya apna name bataye : ")
print(f'\n{name.title()} Ji, bahut bahut swagat hai aapka hamare show me. Chaliye game start karte hain. Wish you all the best !\n{'='*100}')

for serial_number in range(0,15):
    answer = input(f'{'_'*100}\nThe next question for {money_tiers[serial_number]} Rs. is -{questions[serial_number]}\nTypes your answer (A/B/C/D and press enter to lock) : ').upper()

    if answer not in ("A","B","C","D"):
        print("Invalid input, please choose among A/B/C/D only.")
        continue
    
    if answer == answers_option_only[serial_number]:        
        money_collected = money_tiers[serial_number]
        if money_collected == 10000000:
            print(f'\nBravo ! You have successfully won {money_tiers[serial_number]} Rs. Congratulations on becoming a Crorepati. We wish you a bright and rich future ahead.')
            break
        print(f'\nThat is correct answer.\nCongratulations ! You have successfully won {money_collected} Rs. Get ready for the next round')
    else:
        print(f'\nAlas ! This is not the right answer. \n The right answer is {answers[serial_number]}. \nYou have won {money_collected} so far and your journey ends here.\n We wish you a bright future ahead.')
        money_collected += money_tiers[serial_number]
        break
